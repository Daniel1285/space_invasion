            ...
            try:
                ...
                if fer.decrypt(pwd.encode()).decode() == password:
                    ...
                    if RECORD < main_player.kill:
                        collection.update_one({"user_name": login},
                                              {"$set": {"level": main_player.level, "record": main_player.kill}})
        ...
        for event in pygame.event.get():
            ...
            if signup_button_click:
                if user_name != "" and password != "":
                    user = {'user_name': user_name, "password": fer.encrypt(password.encode()).decode(), 'level': 0,
                            'record': 0}
